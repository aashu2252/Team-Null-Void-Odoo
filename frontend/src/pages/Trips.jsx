    ...
    <div className="space-y-6 max-w-full font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold shadow-md transition-all cursor-pointer ${dispatchMode
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {trips.map((t) => (
            <button
              ...
              className={`px-4.5 py-2.5 rounded-xl border text-xs font-bold transition-all cursor-pointer shrink-0 ${activeTripId === t.id
                  ...
                  <div className="relative border-l-2 border-border-custom ml-3 space-y-6 py-1">
                    {activeTrip.stops.map((stop, idx) => (
                      <div key={idx} className="relative pl-6">
                        <div className={`absolute -left-[7px] top-0.5 w-3 h-3 rounded-full ring-4 ring-card-bg ${stop.status === 'Completed' ? 'bg-brand-success' : stop.status === 'Active' ? 'bg-brand-primary' : 'bg-txt-muted'
                          }`} />

                        <div className="flex justify-between items-center">
                          <div>
                            <h5 className={`text-xs font-bold ${stop.status === 'Completed' ? 'text-txt-secondary line-through' : 'text-txt-primary'
                              }`}>
                              {stop.name}
                            </h5>
                          ...
                          <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${stop.status === 'Completed' ? 'bg-brand-success/10 text-brand-success' : stop.status === 'Active' ? 'bg-brand-primary/10 text-brand-primary' : 'bg-surface text-txt-muted'
                            }`}>
                            {stop.status}
                          </span>
                        </div>
