            ...
            {documentChecks.map((doc) => {
              ...
              return (
                <div key={doc.id} className="p-3 bg-surface/40 dark:bg-card-elevated border border-border-custom/50 rounded-2xl flex items-center justify-between">
                  ...
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${doc.status === 'Approved' ? 'bg-brand-success/10 text-brand-success' : 'bg-brand-warning/10 text-brand-warning animate-pulse'
                    }`}>
                    {doc.status}
                  </span>
                </div>
